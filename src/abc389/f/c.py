...
def main():
    ...
    sorted_val = [v for (v, _) in query]

    mapping = lambda f, x: x + f
    composition = lambda f, g: f + g
    seg = LazySegTree(max, -10 ** 9, mapping, composition, 0, sorted_val)

    for i in range(n):
        lft = seg.max_right(0, lambda val: val < l[i])
        rig = seg.max_right(lft, lambda val: val <= r[i])
        if lft < rig:
            seg.apply(lft, rig, 1)
    ans = [0] * q
    for i in range(q):
        ans[query[i][1]] = seg.get(i)
    for i in range(q):
        print(ans[i])
