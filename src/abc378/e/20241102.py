...
def create_matrix(default_value, rows, columns):
...
def main():
    ...
    ccum = [0]
    for i in range(n):
        ccum.append(ccum[-1] + cum[i + 1])  # TODO 良いか

    st = SegTree(aaa, lambda x, y: x + y, 0)
    ans = 0
    for i in range(1, 1 + n):
        ...
        if cm < m - 1:
            ...
            tmp = 0
        dp_r = i * cm - ccum[i - 1] + m * tmp
        ans += dp_r

        current_freq = st.query(cm, cm + 1)
        st.update(cm, current_freq + 1)
