...
def op(a, b):
    a1, a2 = a
    b1, b2 = b
    return (a1 + b1) % MODULO, a2 + b2


def mapping(act, x):
    b, c = act
    val, siz = x
    return (b * val + c * siz) % MODULO, siz


def composition(act1, act2):
    b1, c1 = act1
    b2, c2 = act2
    return (b1 * b2) % MODULO, (b1 * c2 + c1) % MODULO
...
def main():
    ...
    a = INN()
    a = [(ai, 1) for ai in a]
    st = LazySegmentTree(a, op, (0, 0), mapping, composition, (1, 0))
    for _ in range(q):
        t, *q = INN()
        if t == 0:
            l,r,b,c = q
