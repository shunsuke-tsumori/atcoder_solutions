...
# SegTree
#####################################################
from typing import Callable, TypeVar, Generator, Optional, Any, Union
import atcoder._bit
...
def main():
    n, q = INN()
    a = INN()
    mapping = lambda f, e: e if f is None else f
    composition = lambda f1, f2: f1 if f1 is not None else f2
    st = LazySegTree(max, -INF, mapping, composition, None, a)
    for _ in range(q):
        t, x, v = INN()
        if t == 1:
            st.apply(x - 1, x, v)
