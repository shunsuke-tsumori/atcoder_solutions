...
def main():
    ...
    # dp[i]: (いじらない値、0にする最小数、1にする最小)
    dp = [(0, 0, 0)] * len(a)
    for i in range(len(a)):
        dp[i] = (int(a[i]), int(a[i]), 1 - int(a[i]))

    while len(dp) > 1:
        crr_dp = []
        for i in range(0, len(dp), 3):
            v0, v1, v2 = dp[i], dp[i + 1], dp[i + 2]
            cst0 = min(
                v0[1] + v1[1] + min(v2[1], v2[2]),
                v0[1] + v2[1] + min(v1[1], v1[2]),
                v1[1] + v2[1] + min(v0[1], v0[2]),
                v0[1] + v1[1] + v2[1]
            )
            cst1 = min(
                v0[2] + v1[2] + min(v2[1], v2[2]),
                v0[2] + v2[2] + min(v1[1], v1[2]),
                v1[2] + v2[2] + min(v0[1], v0[2]),
                v0[2] + v1[2] + v2[2]
            )
            normal_v = 1 if v0[0] + v1[0] + v2[0] >= 2 else 0
            crr_dp.append((normal_v, cst0, cst1))
        dp = crr_dp
    nv, r0, r1 = dp[0]
    if nv == 0:
        print(r1)
