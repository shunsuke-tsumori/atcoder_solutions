...
def main():
    ...
    if sum(a) != n:
        ...
        return
    x = [xi - 1 for xi in x]
    sorted_pair = [(xi, ai) for (xi, ai) in zip(x, a)]
    sorted_pair = sorted(sorted_pair, key=lambda pair: pair[0])
    if sorted_pair[0][0] != 0:
        print(-1)
        return
    height = sorted_pair[0][1]
    cnt = 0
    for i in range(m - 1):
        dist = sorted_pair[i + 1][0] - sorted_pair[i][0]
        if dist > height:
            print(-1)
            return
        cnt += (dist * (2 * height - dist - 1)) // 2
        height -= dist
        height += sorted_pair[i + 1][1]
    if height > 1:
        cnt += (height - 1) * height // 2
    print(cnt)
    return
