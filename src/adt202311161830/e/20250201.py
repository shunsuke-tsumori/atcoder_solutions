...
def main():
    ...
    fk_point = [-INF] * (m + 1)
    fk_point[0] = 0
    for i in range(1, min(m, len(fk)) + 1):
        fk_point[i] = fk_point[i - 1] + fk[i - 1]
    ...
    for i in range(1, m + 1):
        if hk_head >= len(hk) and kk_head >= len(kk):
            break

        additional = 0
        if crr_kk == 0 and kk_head < len(kk):
            crr_kk += kk[kk_head]
            kk_head += 1
        else:
            if crr_kk > 0:
                if hk_head < len(hk):
                    additional = hk[hk_head]
                    crr_kk -= 1
                hk_head += 1
        hk_point[i] = hk_point[i - 1] + additional
    ans = 0
    for i in range(m + 1):
        crr = fk_point[i] + hk_point[m - i]
        ans = max(ans, crr)
    print(ans)
    return
