...
def main():
    ...
    for l in range(layers):
        ...
        for r in range(l + 1, n - l - 1):
            positions.append((r, n - l - 1))

        for c in range(n - l - 1, l - 1, -1):
            positions.append((n - l - 1, c))

        for r in range(n - l - 2, l, -1):
            positions.append((r, l))

        elements = [grid[x][y] for (x, y) in positions]
        ...
        shift = (k_rot * (total_elements // 4)) % total_elements

        rotated_elements = elements[-shift:] + elements[:-shift]

        for idx, (x, y) in enumerate(positions):
            grid[x][y] = rotated_elements[idx]
