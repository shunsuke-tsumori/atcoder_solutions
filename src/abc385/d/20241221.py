...
def main():
    n, m, sx, sy = INN()
    x, y = IN_2(n)
    dc = [ISS() for _ in range(m)]

    house_x = defaultdict(SortedSet)
    house_y = defaultdict(SortedSet)
    ...
    for i in range(m):
        ...
        if d == "U":
            ...
            ny = sy + c
            crr_set = house_x[nx]
            tgt = list(crr_set.irange(sy, ny))
            cnt += len(tgt)
            for yy in tgt:
                crr_set.discard(yy)
                house_y[yy].discard(nx)
            sy = ny
        if d == "D":
            nx = sx
            ny = sy - c
            crr_set = house_x[nx]
            tgt = list(crr_set.irange(ny, sy))
            cnt += len(tgt)
            for yy in tgt:
                crr_set.discard(yy)
                house_y[yy].discard(nx)
        ...
        if d == "L":
            ...
            ny = sy
            crr_set = house_y[ny]
            tgt = list(crr_set.irange(nx, sx))
            cnt += len(tgt)
            for xx in tgt:
                crr_set.discard(xx)
                house_x[xx].discard(ny)
            sx = nx
        if d == "R":
            nx = sx + c
            ny = sy
            crr_set = house_y[ny]
            tgt = list(crr_set.irange(sx, nx))
            cnt += len(tgt)
            for xx in tgt:
                crr_set.discard(xx)
                house_x[xx].discard(ny)
